...
- **Full-puzzle cache:** generated puzzles are cached so they aren't regenerated on every load/screen-switch/yesterday-view (generation is deterministic per seed). Two tiers: an in-memory map plus `localStorage` (`bloombert-puzzle-YYYY-MM-DD` for daily, `bloombert-timed-puzzle-YYYY-MM-DD` for timed — keyed by the *real* date, so the timed offset is subtracted first). Accessed via `getPuzzle(seed)`/`getTimedPuzzle(seed)` in `puzzle.js`; `generatePuzzle`/`generateTimedPuzzle` stay pure (the verify harness drives them directly). `validWords` is **not** persisted — it's reconstructed as `commonWords.concat(bonusWords)` on read. `hasCachedPuzzle`/`hasCachedTimedPuzzle` probe without generating. `initDailyPuzzle` idle-warms yesterday's daily + today's timed puzzle via `warmNeighborPuzzles()`. Safe to clear; regenerated deterministically.
- **Async load shell:** `initDailyPuzzle`/`initTimedPuzzle` render via `loadPuzzleDeferred(cached, finish, token)`. On a cache hit `finish` runs synchronously; on a miss it shows `#puzzle-loading-overlay`, defers generation by two `requestAnimationFrame`s (so the overlay paints first), and locks input (`puzzleLoading` flag, checked in `appendLetter`/`deleteLetter`/`submitGuess`/`shuffleOuter`). `puzzleLoadToken` is incremented by every init; a deferred run whose captured token no longer matches is dropped (user navigated away mid-generation). For an already-expired timed puzzle, `markTimedEndModal(deferred)` opens `modal-timed-end` directly on the deferred (post-`showScreen`) path but uses `pendingTimedEndModal` on the synchronous path.
...
- Letter tiles fire on `pointerdown` (via `pressTile`), not `click` — fast successive taps had their click events swallowed by mobile double-tap heuristics, dropping letters. A `click` listener remains for keyboard activation only, gated on `e.detail === 0`.
...
- Post-expiry freeze: `expireTimer()` ends with `freezeAfterExpiry()`, which for `EXPIRE_FREEZE_MS=1000` marks `modal-timed-end` `.modal--frozen` (`pointer-events: none`, so backdrop/close/buttons are all inert) and disables its two buttons — a tap already in flight toward Enter can't dismiss the results. Handlers also guard on `expireFrozen`. `clearExpireFreeze()` cancels it early and runs on navigate-away (`showScreen`/`applyShowScreenFromPop`) and in `initTimedPuzzle`.
